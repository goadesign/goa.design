...
### モデルリクエストのローカル検証による拒否

`model.NewRequestValidationError(cause)` は、プロバイダーが受け入れる前に、
アプリケーション側の検証がモデルリクエストを拒否した場合にのみ使用します。
リモートのモデルアダプターは、そのサービスが明示的に返すリクエスト検証エラーから
この型を復元できます。原因は必須です。`Unwrap()` は元のエラーを公開し、
`Error()` は診断の全文を返します。この型には、プロバイダー名、HTTP ステータス、
再試行の設定、回復の指示は含まれません。

ネットワーク、オブザーバー、キャンセル、プロバイダーの失敗には使用しないでください。
既存の検証処理やアダプターは、管理する側がローカルのリクエスト拒否を明示的に
分類しない限り、動作を変えません。実際にプロバイダーが拒否した場合は、引き続き
`model.ProviderError` を使用します。モデルやプランナーの不正な出力は、別の
出力検証の契約に従います。

実行は `model_request`、`Retryable: false` で終了し、プロバイダー、操作、
プロバイダーコード、HTTP ステータスは記録されません。既定の表示メッセージは
“The AI request could not be prepared.” です。アプリケーションはプロセスの
起動時に `hooks.PublicErrorModelRequest` を変更できます。`DebugMessage` は
診断の全文を保持します。ツールの実行やネストしたエージェントが、このエラーを
再試行や修正のための追加モデル呼び出しに変換することはありません。同じプランナー
アクティビティ内の以前のモデル呼び出しがすでに公開したテキストは、実行終了前に
保持されます。アプリケーション独自の Temporal `ApplicationError` を直接返した
場合は、既存の分類と再試行ポリシーが維持されます。この型は、その明示的な外側の
エラーを上書きしません。

Temporal は `goa_ai.request_validation_error` として保存し、
`NonRetryable: true` と、有効な診断の全文をメッセージに保持します。詳細情報や
原因オブジェクトは保存しません。読み取り処理は、再試行可能、詳細情報や原因を含む、
または UTF-8 が無効な保存値を拒否します。既存の診断エンコードと外部の失敗サイズ
制限は引き続き適用され、新しいテキスト上限は追加しません。以前に保存した
プロバイダー、出力、汎用、キャンセルの失敗を再解釈することもありません。

アダプターがこの型を返し始める前に、ワーカーを更新してください。古いワーカーは
保存された分類を読み取れないため、この型を含む履歴を処理してはいけません。
以前のバージョンに戻す場合も同様です。該当する履歴は、更新済みワーカーだけに
割り当ててください。データベースの移行、生成 API の変更、互換モードはありません。
更新しても、過去に保存した失敗の分類は変わりません。
...
新しいプロバイダー、汎用、出力契約、および不正な予約型の Temporal 失敗は、
次の 4 つの内部アプリケーションエラー型を使用します。
...
完了したツール呼び出しは、planner が受け取る前に、提示されたスキーマと、付属する生成済み decoder の検証に合格する必要があります。
フィールドメタデータにより、必須項目、型、列挙値、配列長の独立した違反を説明できます。
配列の境界値は許容され、制限は提出された配列に適用されます。実行全体の上限ではありません。
例：`Field "items" must contain at most 3 items.`

runtime は `allOf` を含む独立した必須制約と、有効な識別子で選択されたユニオンの分岐だけをたどります。
`anyOf` の代替分岐、`contains` の候補、プロパティ名の検査を、対応するすべての値を変更する指示には変換しません。
配列の添字とマップのキーは `*` で表示します。未宣言のフィールドは、提出された名前を繰り返さずに親オブジェクト上の誤りとして示します。

指示は並べ替え、重複を除去します。同じ表示パスに異なる指示がある場合、制約を統合しようとせず、その指示群を省略します。
未対応、曖昧、または長すぎる指示があっても、他のフィールドへの有用な指示は保持します。
説明文と列挙値を含む各指示は、バイト上限内に全文が収まる場合だけ含めます。
一部を省いた修正指示には、他のエラーを詳述していない旨を添えます。一つも含められない場合は一般的な指示を使います。
引数は実行前に拒否されたままであり、Goa-AI は分割、切り詰め、書き換えを行いません。
元のエラー、受理規則、修正ターン数の上限は変わりません。

スキーマの検証や型付きの引数検証による拒否が修正の対象となる場合、モデルクライアントは
フィールドの修正指示の後に、検証済みの入力例を省略せずに追加できます。入力例はモデルを
呼び出す前にリクエストの検証契約と一緒にコピーされるため、後からリクエストを変更しても
置き換わりません。添える指示では、例のサンプル値をそのままコピーするのではなく、
リクエストに適した値と、ユニオンで許可された分岐を選ぶようモデルに求めます。

修正指示全体は、拒否されたモデル呼び出し1回あたりの既存の上限である4,096バイト以内に
収まる必要があります。指示文、入力例、UTF-8のバイト数をすべて含みます。例がない場合や
大きすぎる場合は、既存のフィールド修正指示を変えずに例全体を省略し、途中で切り詰めません。
これは任意の修正コンテキストの上限であり、引数の上限ではありません。検証、拒否された
応答の診断情報、受理済みの履歴、選択可能なツール、修正ターン数の上限は変わりません。
引数を自動修復したり、再試行を追加したりすることもありません。
詳細は[フレームワークの契約](https://github.com/goadesign/goa-ai/blob/main/docs/runtime.md#model-visible-tool-arguments)を参照してください。
