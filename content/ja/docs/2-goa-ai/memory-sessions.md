...
- **プランナー**: `planner.PlanInput` または `planner.PlanResumeInput` の `PrepareMessages()` を呼び、エラーを確認してから準備済みのトランスクリプトを使います。追加の状態を持ち回らず、メッセージにもとづいて判断できます。詳細は[メッセージの準備](../runtime/#preparing-conversation-messages)を参照してください。
...
- **状態はランタイムに持たせる**: プランナー内で「ツール履歴」配列や「以前のメッセージ」スライスを並行して維持しないでください。`PrepareMessages()` を呼び、エラーを確認して準備済みのメッセージを使い、新しいパーツの追加はランタイムに任せます。
