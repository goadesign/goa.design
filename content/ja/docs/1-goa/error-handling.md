...
API レベルのエラーは、再利用可能な定義と、デフォルトのトランスポート対応を宣言する場所です。すべてのサービスメソッドがそのエラーを返すという意味ではありません。生成されたエンドポイントから返せるようにするには、サービスまたはメソッドが `Error("invalid_argument")` でそのエラーを選択する必要があります。
