...
### 以前のストリーミングクライアントをアップグレードする

現在の Goa が生成するストリーミングクライアントは、リクエストメタデータで
プロトコルを識別し、メソッドに `Payload` と `StreamingPayload` の両方がある
場合は型付きエンベロープを送ります。以前のプロトコルで生成されたクライアントを
一時的に受け入れるには、API、サービス、またはメソッドのレベルで次のメタデータを
追加します。
...
最初にサーバーを、次にクライアントをアップグレードし、すべてのクライアントが
現在のプロトコルを使うようになったらこのメタデータを削除してください。この
互換モードで対応する以前のクライアントの単発ペイロードは、プリミティブ値または
プリミティブ値の配列に限られます。オブジェクト、マップ、またはユニオンの
ペイロードには現在のプロトコルを使用してください。
