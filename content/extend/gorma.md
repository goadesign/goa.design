...
## Overview
goa is a DSL for the end-user expierence of your API.  It describes the inputs and outputs in exact detail.  `gorma` extends this experience to the data storage layer.  `gorma` adds a new DSL to describe your storage layer and the relationships between your models.  Your `gorma` specification lives in the same `design` folder where the rest of your goa DSL is.  Here's an example:
...
### Storage Group
The StorageGroup specification is the root of your `gorma` configuration.  On it's own it creates nothing, but it serves as the container for all of the storage specifications for your project.  There may be only one StorageGroup defined in your project.  As with nearly all the DSL in goa and gorma, you can add a `Description("...")` specification to your StorageGroup.
...
* Gorma is currently tested extensively against PostgreSQL.  It should work well against other databases supported by Gorm, and if it doesn't please file a bug.
