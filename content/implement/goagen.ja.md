...
`main` コマンドによって生成された Scaffold コードは、すぐに開始する方法として一度生成されます。 このコードは、編集、テストされ、手動で書かれた他のファイル同様一般的な処理が行われることを意味します。デフォルトでは `goagen` は Scaffold ファイルがすでに存在する場合には上書きしませんが、`--regen` 引数をつければ Scaffold ファイルを更新することが出来ます。 `client` コマンドによって生成されるクライアントツールの `main` パッケージも Scaffold です。
...
注: `--regen` を機能させるためには、コメント間に **必ず** コントローラの実装を置かなければなりません。
...
再生成されるコードは、保持すべき 非Scaffold なコードを見つけるために、これらのコメントに依存しています。
