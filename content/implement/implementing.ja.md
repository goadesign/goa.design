...
各サービスごとに Goa はトランスポート階層、エンドポイント階層、そしてサービス層を生成します。
...
最後に、サービス階層はビジネスロジックのある階層です。Goa は各サービスのインターフェースを生成し、ユーザーはそれらの実装して提供します。
...
`formatter` は、サービスメソッドでエンコーディングの前にエラーを返す際に Goa がどのようにそのエラーを整形するか、をオーバーラードすることができます。
...
構造体のフィールドはパブリックなので、ユーザーコードでどのエンドポイントもオーバーラードすることができます：
...
ハンドラはパブリックなフィールドなので、ユーザーコードでいずれのハンドラもオーバーラードすることができます。
