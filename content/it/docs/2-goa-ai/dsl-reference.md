...
superiore dichiarato è la fonte degli esempi di livello superiore esposti al
provider. Codegen conserva quell'esempio nella specifica generata come JSON
grezzo e come oggetto di input analizzato, ed emette anche lo schema annotato e
uno schema con solo l'`example` radice rimosso.

Gli adattatori dei provider usano direttamente queste proiezioni. I provider che
consumano annotazioni di schema usano il JSON Schema annotato. Anthropic diretto
e Bedrock Claude usano `input_examples` nativi con la proiezione senza esempio
radice; Bedrock trasmette i campi Anthropic tramite
`additionalModelRequestFields` quando lo richiede il contratto beta. Gli esempi
generati o sintetizzati non vengono mai promossi a esempi di provider di livello
superiore.
...
**Compressione (riepilogo assistito da modello):**

La compressione riassume i turni più vecchi con un modello mantenendo una coda esatta e limitata di turni recenti completi. Separa due decisioni:

- `CompressAtTurns(n)` e `CompressAtMaxInputTokens(n)` decidono quando eseguire il riepilogo. Se entrambi sono configurati, uno dei due trigger può avviare la compressione.
- `KeepMaxTurns(n)` e `KeepMaxInputTokens(n)` decidono quali turni completi più recenti restano esatti dopo il riepilogo. Se entrambi sono configurati, si applicano entrambi i limiti.

I budget di token vengono contati a runtime dal modello di cronologia configurato, perché la tokenizzazione dipende dal modello. `KeepMaxInputTokens` non tronca mai un turno: il runtime procede all'indietro dal turno più recente e conserva solo turni completi che rientrano nel budget.
...
- `CompressAtTurns(n)`: attiva la compressione quando sono presenti almeno `n` turni logici.
- `CompressAtMaxInputTokens(n)`: attiva la compressione quando il transcript visibile al provider supera `n` token di input.
- `KeepMaxTurns(n)`: conserva esatti al massimo `n` turni completi più recenti.
- `KeepMaxInputTokens(n)`: conserva turni completi recenti il cui conteggio di token di input rientra in `n`.

La compressione richiede almeno un trigger `CompressAt...` e almeno un budget di mantenimento `KeepMax...`.

**Requisito del modello storico:**

Quando si utilizza la compressione, è necessario fornire un `model.Client` tramite il campo `HistoryModel` generato nella configurazione dell'agente. Il runtime utilizza questo client con `ModelClassSmall` per riepilogare i turni precedenti e, quando è configurato un budget di token, contare la richiesta visibile al provider. La compressione basata sui token richiede che il modello di cronologia implementi `model.TokenCounter` con conteggi esatti; l'adapter Bedrock lo fa con l'API nativa `CountTokens` di Bedrock.
...
I valori DSL sono predefiniti generati. Un deployment può sostituirli quando il modello scelto ha una finestra di contesto o un budget operativo diverso:
...
Se `HistoryModel` non viene fornito quando è configurata la compressione, la registrazione avrà esito negativo.
...
Consuma un `Toolset(FromRegistry(...))` nominato con `Use`, oppure direttamente un intero `Registry`. Aggiungi `Deferred()` in `Use` per la ricerca nativa. L’agente generato risolve i contratti attuali una volta per attività di pianificazione. Collega client del registro distribuito e Pulse già costruiti con `rt.RegisterRegistry`; `Definition()` e `NewClient(rt)` non ricevono cataloghi.

I provider pubblicano `ToolSchemas()` con conferma, paginazione, metadati e dati server. Gli strumenti dinamici di servizio usano quei contratti; agenti e controllo restano compilati. Le fonti nominate devono esistere e rispettare la versione fissata; un registro completo può essere vuoto. Consumi duplicati o sovrapposti e strumenti inline o esportati su riferimenti al registro sono rifiutati.
