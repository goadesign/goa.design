...
Goa-AI supporta anche contratti `Completion(...)` di proprieta del servizio per
output diretti e tipizzati dell'assistente. Quando esegui `goa gen`, questi
contratti generano codice sotto `gen/<service>/completions`.

I nomi delle completion fanno parte del contratto: 1-64 caratteri ASCII, solo
lettere/cifre/`_`/`-`, e devono iniziare con una lettera o una cifra. Il codice
generato include helper unary (`Complete<Name>(...)`) e di streaming
(`StreamComplete<Name>(...)`, `Decode<Name>Chunk(...)`). I frammenti
`completion_delta` sono solo anteprime; un unico chunk finale `completion` e
canonico.
Lo schema generato rimane il contratto di servizio canonico; gli adattatori
del modello possono normalizzarlo a un sottoinsieme specifico del provider
per il decoding vincolato, ma devono rifiutare esplicitamente i provider che
non possono rappresentare il contratto dichiarato.
...
| `Bookkeeping` | Tool | Marca lo strumento come bookkeeping: le chiamate non consumano il budget run-level `MaxToolCalls` e restano nascoste ai futuri turni del planner per impostazione predefinita |
| `PlannerVisible` | Tool | Mantiene visibile al turno successivo del planner un risultato di bookkeeping non terminale |
| **Funzioni di policy** | | | |
...
| **MCP Functions** | | |
| `MCP` | Service | Abilita il supporto MCP |
| `ProtocolVersion` | MCP option | Imposta la versione del protocollo MCP |
| `Tool` | Method | Contrassegna un metodo come strumento MCP in un servizio con MCP abilitato |
| `Toolset(FromMCP(...))` | Top-level | Dichiara un toolset MCP derivato da un servizio Goa |
| `Toolset("name", FromExternalMCP(...), func() { ... })` | Top-level | Dichiara un toolset MCP esterno con schemi inline |
...
Questo è opzionale: quando non è configurato alcun contenuto di prompt lato consumer,
il runtime usa il payload JSON canonico del tool come messaggio utente del run figlio,
e i planner del provider possono renderizzare i propri prompt con contesto iniettato
dal lato server.
...
- Aiutanti di registrazione consapevoli di MCP quando un `Toolset(FromMCP(...))` è referenziato tramite `Use`
...
- Un riferimento `Toolset(FromMCP(...))`
...
In fase di esecuzione, gli artefatti UI (proiettati dai server-data opzionali) sono disponibili su `planner.ToolResult.ServerData`:
...
2. Validati come campi `String` obbligatori nel payload del metodo associato
3. Popolati da `runtime.ToolCallMeta` tramite gli esecutori generati, con hook opzionali `ToolInterceptor.Inject`
...
In fase di esecuzione, gli esecutori di servizio generati copiano i valori corrispondenti da `runtime.ToolCallMeta` prima di eseguire interceptor tipizzati opzionali:
...
- Per impostazione predefinita, i risultati bookkeeping riusciti restano nascosti ai futuri turni del planner. Aggiungi `PlannerVisible()` quando uno strumento di bookkeeping emette uno stato canonico su cui il turno successivo deve ragionare.
...
- Un turno composto solo da bookkeeping deve risolversi nello stesso turno (`TerminalRun()`, `FinalResponse`, `FinalToolResult`, attesa/pausa) oppure produrre almeno un risultato bookkeeping planner-visible che giustifichi la ripresa successiva.
...
`PlannerVisible()` mantiene visibile ai futuri turni del planner il risultato di uno strumento di bookkeeping. Usalo per strumenti del control plane che emettono stato canonico, ad esempio uno snapshot strutturato di avanzamento che deve guidare il prossimo `PlanResume`.
...
- `PlannerVisible()` è valido solo su strumenti di bookkeeping non terminali.
- Le esecuzioni riuscite vengono reinserite nel transcript visibile al modello e nei futuri `PlanResumeInput.ToolOutputs`.
- I fallimenti bookkeeping ritentabili restano planner-visible anche senza `PlannerVisible()`.
- Gli strumenti con budget non hanno bisogno di `PlannerVisible()` perché sono già planner-visible per impostazione predefinita.
...
`MCP(name, version, opts...)` abilita il supporto MCP per il servizio corrente. Configura il servizio per esporre strumenti, risorse e richieste tramite il protocollo MCP.
...
`ProtocolVersion(version)` configura la versione del protocollo MCP supportata dal server. Restituisce una funzione di configurazione da usare con `MCP`.

**Contesto**: Argomento opzionale di `MCP`
...
`Tool(name, description)` contrassegna il metodo corrente come strumento MCP. Il payload del metodo diventa lo schema di input dello strumento e il risultato diventa lo schema di output.
...
`Toolset(FromMCP(service, toolset))` dichiara un insieme di strumenti definiti da MCP e derivati da un server MCP Goa.
