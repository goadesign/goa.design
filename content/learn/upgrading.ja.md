...
これでおしまいです！
v3 の `goa` ツールは後方互換性があり、v2 のデザインに対してコードを生成することができることにも注意してください。
これにより、v2 を GOPATH に保持し、v3 を Go モジュールで利用し、v2 と v3 の両方のプロジェクトで同時に作業することが可能になります。
...
> このドキュメントの残りの部分では v3 で解説していますが、両方に適用できます。
