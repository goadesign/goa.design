...
### Upgrading Earlier Streaming Clients

Current Goa-generated streaming clients identify the stream protocol in request
metadata and send a typed envelope when a method has both `Payload` and
`StreamingPayload`. A server can temporarily accept clients generated by the
earlier protocol by adding this metadata at the API, service, or method level:

```go
Meta("grpc:stream:compat", "v1")
```

Upgrade servers first, then clients, and remove the metadata after every client
uses the current protocol. This compatibility mode only supports earlier
clients whose one-time method payload contains primitive values or arrays of
primitive values; use the current protocol for object, map, or union payloads.
