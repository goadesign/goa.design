...
In other words Google Cloud Endpoints can be completely configured using an OpenAPI spec, one for
...
aannnnd it almost worked... The tool complains that the OpenAPI spec is invalid which is strange
...
file servers in the design which generates `file` parameters in the OpenAPI spec. I decided to
...
Now that we know it's possible to deploy to Endpoints using a goa generated OpenAPI spec let's
...
Endpoints requires using OpenAPI spec extensions to specify the JWT issuer and token retrieval URI
(`x-issuer` and `x-jwks_uri` respectively). It also adds a `x-security` field on path objects used
for specifying the expected value of the JWT token `aud` field. Good timing as support for OpenAPI
...
OpenAPI spec consumed by Endpoints must make use of `oauth2`.
...
and Endpoints. The OpenAPI spec for the endpoints example service is available at
...
fields must match the values specified in the OpenAPI spec. These values are set in the design using
